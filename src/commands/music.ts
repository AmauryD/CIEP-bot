...
export const action: CommandAction = async function (
  ...
  if (originalMessage.guild?.me?.voice.channel) {
    if (isStop === "stop") {
      ...
      await originalMessage.reply(
        "Je suis déjà en train de jouer quelque chose, pour m'arrêter faites la commande suivie de stop"
      );
    }
    return;
  }
  ...
  const soundHandler = voiceConnection.play(
    path.join(process.cwd(), "/static/hymne.mp3")
  );
